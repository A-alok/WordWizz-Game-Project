...
public class VegetableGrid {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(VegetableGrid::new);
    }

    public VegetableGrid() {
        // Initialize vegetable names
        vegetables = new String[]{

                "Pea", "Yam", "Okra", "Kale", "Bean", "Corn", "Beet", "Taro", "Leek", "Chard", "Onion", "Carrot", "Radish", "Garlic", "Potato", "Tomato", "Pepper", "Spinach", "Celery", "Cabbage", "Squash", "Turnip", "Ginger", "Chili", "Beetroot", "Pumpkin", "Lettuce", "Fennel", "Brinjal", "Bitter", "Drumstick", "Cucumber", "Cauliflower", "Broccoli"
        };
    ...
    private void startNewGame() {
        ...
        correctWord = vegetables[random.nextInt(vegetables.length)].toUpperCase(); // Select a random vegetable and convert to uppercase
    ...
    private void initializeGrid() {
        for (int i = 0; i < ROWS; i++) {
            for (int j = 0; j < COLS; j++) {
                ...
                // Add a mouse listener to ignore clicks on the grid
                grid[i][j].addMouseListener(new MouseAdapter() {
                    @Override
                    public void mouseClicked(MouseEvent e) {
                        // Prevent any action when the grid cell is clicked
                        frame.requestFocusInWindow(); // Refocus to capture keyboard input
                    }
                });
            }
        }
    ...
    private void createAndShowGUI() {
        frame = new JFrame("Vegetable Grid");
        ...
        JPanel backgroundPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                ImageIcon backgroundImage = new ImageIcon("C:\\Users\\vinay\\Downloads\\miniiiiiiiiii\\miniiiiiiiiii\\icon\\2.png"); // Set your vegetable background image path
    ...
    private void checkGuess(String guess) {
        ...
        if (guess.equals(correctWord)) {
            showEndGamePopup("Congratulations! You've guessed the vegetable!");
        } else {
            currentRow++;
            currentCol = 0;

            if (currentRow >= ROWS) {
                showEndGamePopup("Game Over! The correct vegetable was: " + correctWord);
            }
        }
    }
